...
#     def _decorator(fn):
#         wrapped = with_guardrails(fn)            # guardrails first
#         return njit(parallel=parallel,           # numba second
#                     cache=cache)(wrapped) if _USE_NUMBA else wrapped
#     return _decorator

def _jit(parallel: bool = False, cache: bool = True):
    def _decorator(fn):
        # Only apply njit, not with_guardrails, for JIT
...
class Integrator:
    ...
    def odeint_dp(self,
        ...
        """
        return self._odeint_dp_impl(y, x1, x2, eps, h1, hmin, D, integ, jacobn)

    @_jit()
    def _odeint_dp_impl(self,
        y: Annotated[np.ndarray, np.float64],  # in/out solution vector
        x1: Annotated[float,    np.float64],    # start of interval
        x2: Annotated[float,    np.float64],    # end of interval
        eps: Annotated[float,   np.float64],    # error tolerance
